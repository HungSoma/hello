...
class UserForm(forms.ModelForm):
    email = forms.EmailField(label='Email address', widget=forms.TextInput(
        attrs={'cols': 80 ,'rows': 20,'size': 20, 'class': "form-control input-md", 'placeholder': "Email"}))
    class Meta:
        model = User
        fields = [ 'email']
class ProfileForm(forms.ModelForm):
    first_name = forms.CharField(label = 'First Name', required = False, widget = forms.TextInput(attrs={'size': 20,'class': "form-control", 'placeholder': 'First Name'}))
    last_name = forms.CharField(label='Last Name', required=False, widget=forms.TextInput(attrs={'size': 20, 'class': "form-control", 'placeholder': 'Last Name'}))
    #gender = forms.TypedChoiceField(label = 'Gender',required = False,widget=forms.Select(attrs={"name": "select_0","class": "form-control"}) )
    #phone = forms.CharField(label = 'Phone', required = False, widget = forms.TextInput(attrs={'size': 20, 'class': 'form-control', 'placeholder': 'Phone Number'}))
    birthday = forms.DateField(input_formats = ['%Y-%m-%d','%m/%d/%Y','%m/%d/%y','%d/%m/%y','%d/%m/%Y'],label ='Your Birthday',required = False, widget = forms.TextInput(attrs ={'size': 20, 'class': " form-control", 'placeholder':"dd/mm/yyyy"}))
    country = forms.CharField(label = 'Your Country', required = False,widget = forms.TextInput(attrs ={'size': 20, 'class': " form-control", 'placeholder':"Viet Nam, Korea, Japan,..."}))
    city = forms.CharField(label = 'City',required = False,widget = forms.TextInput(attrs ={'size': 20, 'class': " form-control", 'placeholder':"Ho Chi Minh, New York, Seoul, Tokyo,..."}))
    organization = forms.CharField(required = False,widget = forms.TextInput(attrs ={'size': 20, 'class': " form-control", 'placeholder':"Organization"}))
